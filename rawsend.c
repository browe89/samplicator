...
static uint16_t udp_sum_calc(uint16_t, uint32_t, uint16_t, uint32_t, uint16_t,
                             const void *);

int raw_send_from_to(s, msg, msglen, saddr, daddr, ttl, flags) int s;
const void *msg;
size_t msglen;
...
int flags;
{
  ...
  uh.uh_sum =
      flags & RAWSEND_COMPUTE_UDP_CHECKSUM
          ? udp_sum_calc(msglen, ntohl(saddr->sin_addr.s_addr),
                         ntohs(saddr->sin_port), ntohl(daddr->sin_addr.s_addr),
                         ntohs(daddr->sin_port), msg)
          : 0;
  ...
  /* Depending on the target platform, te ip_off and ip_len fields
    ...
    (defined(__OpenBSD__) && (OpenBSD > 199702))
  ...
  if (sendto(s, msgbuf, length, flags, (struct sockaddr *)&dest_a,
             sizeof dest_a) == -1)
#endif /* not HAVE_SYS_UIO_H */
  {
    if (getsockopt(s, SOL_SOCKET, SO_ERROR, (char *)&sockerr, &sockerr_size) ==
        0) {
...
extern int make_raw_udp_socket(sockbuflen) int sockbuflen;
{
  int s;
...
uint16_t udp_sum_calc(uint16_t len_udp, uint32_t src_addr, uint16_t src_port,
  ...
  /* if we have an odd number of bytes in the data payload, then set the pad to
   * 1 for special processing
   */
  if (len_udp % 2 != 0) {
    pad = 1;
  }
  ...
  udp_len_total =
      len_udp + 8; /* length sent is length of data, need to add 8 */
  ...
  /* Add all 16 bit words to the sum, if pad is set (ie, odd data length) this
   * will just read up to the last full 16 bit word.
   * */
  ...
  /* ok, if pad is true, then the pointer is now  right before the last single
   * byte in the payload.  We only need to add till the end of the string
   * (1-byte) , not the next 2 bytes as above.
   */
  if (pad) {
    sum += ntohs(*(unsigned char *)buff);
  }

  /* keep only the last 16 bits of the 32 bit calculated sum and add the carry
   * overs */
  while (sum >> 16) {
