...
static int send_pdu_to_peer(struct peer *, const void *, size_t,
                            struct sockaddr_in *);
static void parse_args(int, char **, struct samplicator_context *,
                       struct source_context *);
static int parse_peers(int, char **, struct samplicator_context *,
                       struct source_context *);
...
static const char *my_inet_ntoa(const struct in_addr *in) {
  unsigned a = ntohl(in->s_addr);
  static char buffer[16];
  sprintf(buffer, "%d.%d.%d.%d", (a >> 24) & 0xff, (a >> 16) & 0xff,
          (a >> 8) & 0xff, a & 0xff);
  return buffer;
}
...
static void parse_args(argc, argv, ctx, sctx) int argc;
char **argv;
struct samplicator_context *ctx;
struct source_context *sctx;
{
  ...
  while ((i = getopt(argc, argv, "hVb:d:p:s:x:c:fSn")) != -1) {
    ...
    case 'p': /* flow port */
      ...
      if (ctx->fport < 0 || ctx->fport > 65535) {
        fprintf(stderr, "Illegal receive port %d - \
...
static int parse_peers(argc, argv, ctx, sctx) int argc;
char **argv;
struct samplicator_context *ctx;
struct source_context *sctx;
{
  ...
  if (!(sctx->peers =
            (struct peer *)malloc(sctx->npeers * sizeof(struct peer)))) {
  ...
  TTL:
    if ((*c == TTL_SEPARATOR) && (*(c + 1) > 0)) {
      *c = 0;
      ...
      if (sctx->peers[i].ttl < 1 || sctx->peers[i].ttl > 255) {
        fprintf(stderr,
      ...
      if (raw_sock == -1) {
        if ((raw_sock = make_raw_udp_socket(ctx->sockbuflen)) < 0) {
          if (errno == EPERM) {
            fprintf(
                stderr,
                "Not enough privilege for -S option---try again as root.\n");
          } else {
...
static void read_cf_file(file, ctx) const char *file;
struct samplicator_context *ctx;
{
  FILE *cf;
...
static int init_samplicator(ctx) struct samplicator_context *ctx;
{
  ...
  if (setsockopt(ctx->fsockfd, SOL_SOCKET, SO_RCVBUF, (char *)&ctx->sockbuflen,
                 sizeof ctx->sockbuflen) == -1) {
    fprintf(stderr, "setsockopt(SO_RCVBUF,%d): %s\n", ctx->sockbuflen,
            strerror(errno));
  }
  if (bind(ctx->fsockfd, (struct sockaddr *)&local_address,
...
static int samplicate(ctx) struct samplicator_context *ctx;
{
  ...
  if (ctx->fork == 1) {
    ...
    if (pid == -1) {
      ...
      exit(0);
    } else { /* end interaction with shell */
      fclose(stdin);
      fclose(stdout);
      fclose(stderr);
    }
  ...
  while (1) {
    ...
    if ((n = recvfrom(ctx->fsockfd, (char *)fpdu, sizeof(fpdu), 0,
...
static void usage(progname) const char *progname;
{
  announce_version();
...
",
          progname, FLOWPORT, (unsigned long)DEFAULT_SOCKBUFLEN, PORT_SEPARATOR,
          FREQ_SEPARATOR, TTL_SEPARATOR, FLOWPORT, DEFAULT_TTL);
}

static int send_pdu_to_peer(peer, fpdu, length, source_addr) struct peer *peer;
const void *fpdu;
size_t length;
struct sockaddr_in *source_addr;
{
  if (peer->flags & pf_SPOOF) {
    ...
    return raw_send_from_to(peer->fd, fpdu, length, source_addr, &peer->addr,
...
static int make_cooked_udp_socket(sockbuflen) int sockbuflen;
{
  int s;
  if ((s = socket(PF_INET, SOCK_DGRAM, 0)) == -1)
    return s;
  if (sockbuflen != -1) {
    if (setsockopt(s, SOL_SOCKET, SO_SNDBUF, (char *)&sockbuflen,
                   sizeof sockbuflen) == -1) {
